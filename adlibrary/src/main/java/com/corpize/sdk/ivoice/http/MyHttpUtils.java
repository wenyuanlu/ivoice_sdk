...
import com.corpize.sdk.ivoice.CorpizeJ;
import com.corpize.sdk.ivoice.common.Constants;
import com.corpize.sdk.ivoice.http.callback.BaseCallback;
import com.corpize.sdk.ivoice.http.util.ApiException;
import com.corpize.sdk.ivoice.http.util.ErrorCustom;
import com.corpize.sdk.ivoice.http.util.HttpLogUtil;
import com.corpize.sdk.ivoice.utils.AESUtil;
...
public class MyHttpUtils {
    ...
    private static final int                 TIMEOUT_READ_MILLIONS    = 5000;//上传超时时间
    private static final int                 TIMEOUT_WRITE_MILLIONS   = 6000;//返回超时时间
    ...
    private              byte[]              mGzipParam;            //传递的Gzip数据
    private              boolean             mIsEncrypt               = false;//是否开启请求加密
    private              boolean             mIsEncrypts              = false;//是否开启请求加密,so加密
    ...
    private void doPostJson (String url, final BaseCallback callback) {
        ...
        if (mIsEncrypts) {
            try {
                //先加密
                String back = new CorpizeJ().handleA(mJsonParam);
                //解析
                JSONObject jsonBack = new JSONObject(back);
                String     encrpty  = jsonBack.getString("data");
                int        len      = jsonBack.getInt("len");
                sign = jsonBack.getString("sign");
                //判断加密返回数据
                if (TextUtils.isEmpty(encrpty) || TextUtils.isEmpty(sign) || encrpty.length() != len) {
                    sendFailResultCallback(url, new ApiException(result, QcErrorCode.NET_ERROR_OTHER_LENTH_ERRPR.errorCode),
                            QcErrorCode.NET_ERROR_OTHER_LENTH_ERRPR.errorCode, null, callback);
                    return;
                }

                //Gzip压缩需要用到
                mGzipParam = back.getBytes();
            } catch (Exception e) {
                sendFailResultCallback(url, e, QcErrorCode.NET_ERROR_OTHER_ERRPR.errorCode, null, callback);
        ...
        try {
            ...
            conn.setConnectTimeout(TIMEOUT_CONNECT_MILLIONS);
            conn.setRequestProperty("accept", "*/*");//接收所有类型("*/*")  接收json类型("application/json")
            ...
            if (mIsEncrypts) {
                ...
                outStream.write(mGzipParam);
                outStream.flush();
                outStream.close();
            } else {
                //发送Json请求数据,一定要用BufferedReader 来接收响应， 使用字节来接收响应的方法是接收不到内容的
            ...
            int    responseCode = conn.getResponseCode();
            String contentType  = conn.getContentType();
            String headerField  = conn.getHeaderField("Accept-Encoding");
            if (responseCode == HttpURLConnection.HTTP_OK) {
                if (mIsEncrypts) {
                    //解析Gzip数据
                    if (!TextUtils.isEmpty(headerField) && headerField.equals("gzip")) {
                        gzipReader = new GZIPInputStream(conn.getInputStream());
                        int          len     = 0;
                        byte[]       gBuffer = new byte[1024];
                        StringBuffer sb      = new StringBuffer();
                        while ((len = gzipReader.read(gBuffer)) >= 0) {
                            sb.append(new String(gBuffer, 0, len));
                        }
                        result = sb.toString();
                    } else {
                        // 读取响应 解析json数据
                    ...
                    if (!TextUtils.isEmpty(result)) {
                        try {
                            JSONObject jsonBack = new JSONObject(result);
                            String     data     = jsonBack.getString("data");
                            sign = jsonBack.getString("sign");
                            //判断加密返回数据
                            ...
                            //解密
                            String back = new CorpizeJ().handleB(data, sign);
