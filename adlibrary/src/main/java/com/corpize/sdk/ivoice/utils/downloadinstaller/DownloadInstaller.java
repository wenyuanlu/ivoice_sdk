...
import com.qichuang.annotation.StringRes;
import com.qichuang.core.app.NotificationCompat;
import com.qichuang.core.collection.ArrayMap;
...
public class DownloadInstaller {
     ...
     * @param callBack       进度状态回调
     */
    public DownloadInstaller (int position, Activity context, String downloadApkUrl, DownloadProgressCallBack callBack) {
        this(position, context, downloadApkUrl, false, callBack);
    }
     ...
     * @param callBack                 回调
     */
    public DownloadInstaller (int position, Activity context, String downloadApkUrl, boolean isForceGrantUnKnowSource, DownloadProgressCallBack callBack) {
        this.mContext = context;
    ...
    public void start () {
        ...
        if (downloadStatus == null || downloadStatus == UpdateStatus.UN_DOWNLOAD || downloadStatus == UpdateStatus.DOWNLOAD_ERROR) {
            ...
            ThreadManagerUtil.getDefaultProxy().execute(mDownApkRunnable);
        ...
        public void run () {
            ...
            try {
                ...
                if (e instanceof FileNotFoundException) {
                    notifyError(getStringFrom(R.string.qcad_download_failure_file_not_found));
                    toastError(R.string.qcad_download_failure_file_not_found);
                } else if (e instanceof ConnectException) {
                    notifyError(getStringFrom(R.string.qcad_download_failure_net_deny));
                    toastError(R.string.qcad_download_failure_net_deny);
                    ...
                    notifyError(getStringFrom(R.string.qcad_download_failure_storage_permission_deny));
                    toastError(R.string.qcad_download_failure_storage_permission_deny);
                } else {
                    notifyError(getStringFrom(R.string.qcad_apk_update_download_failed));
                    toastError(R.string.qcad_apk_update_download_failed);
                }
    ...
    public void installProcess () {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (canInstallPackage) {
                ...
                mContext.startActivityForResult(intent, mPosition);
            }
        } else {
            installApk();
            downLoadStatusMap.put(downloadApkUrlMd5, UpdateStatus.UN_DOWNLOAD);
    ...
    private void initNotification () {
        ...
        builder.setContentTitle(mContext.getResources().getString(R.string.qcad_apk_update_tips_title)) //设置通知标题
                .setSmallIcon(R.drawable.qcad_download)
                .setDefaults(Notification.DEFAULT_LIGHTS) //设置通知的提醒方式： 呼吸灯
                .setPriority(NotificationCompat.PRIORITY_MAX) //设置通知的优先级：最大
                .setAutoCancel(true)  //
                .setOngoing(true)     // 不可以删除
                .setContentText(mContext.getResources().getString(R.string.qcad_apk_update_downloading_progress))
    ...
    private void notifyError (String errorMsg) {
        builder.setContentTitle(mContext.getResources().getString(R.string.qcad_apk_update_tips_error_title));
    ...
    private void updateNotify (int progress) {
        ...
        builder.setContentText(mContext.getResources().getString(R.string.qcad_apk_update_downloading_progress) + " 「" + progress + "%」");
        notification = builder.build();
